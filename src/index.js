...
window.musicStreams = {}; // array of connected clients for three.js scene
...
function initSocketConnection() {
	return new Promise(resolve => {
		...
		socket.on('introduction', (_id, _ids, _music_streams, _server_music_offset, _server_sync_time) => {

			// keep a local copy of my ID:
			console.log('My socket ID is: ' + _id);
			mySocketID = _id;

      yorbScene.setServerMusicOffset(_server_music_offset, _server_sync_time);
      ...
      if (window.location.href.indexOf('nomusic') < 0) {
        for (var key in _music_streams) {
          addMusicStream(_music_streams[key]);
        }
      } else {
        console.log('[Music] No music');
      }
			resolve();
		});
    ...
    socket.on('playSteam', (_ms_id, currentSong) => {
      console.log('[Music] Instructions to play ' + _ms_id);
      yorbScene.playStream(_ms_id, currentSong);
    });
...
async function addMusicStream(_ms) {
	console.log("[Music] Adding music stream with id " + _ms.id);
	musicStreams[_ms.id] = _ms;
	yorbScene.addMusicStream(_ms);
}
...
function setupControls() {
  var webcamImage = document.getElementById("webcam-status-image")
  webcamImage.addEventListener('click',
    function (evt) {
      toggleWebcamVideoPauseState();
    }
  , false)
  var micImage = document.getElementById("microphone-status-image")
